...
namespace TylerCode.SoundSystem
{
    ...
    public class S4SoundPlayer : MonoBehaviour
    {
        ...
        public void StartPlayer(SoundPlayerSettings player, int id, S4SoundManager manager, bool fadeIn = false)
        {
            ...
            if (player.setPitch)
            {
                audioSource.pitch = player.pitch;
                // Debug.Log("SET PITCh");
            }

            else if (player.randomPitch && !player.setPitch)
            {
    ...
    public class SoundPlayerSettings
    {
        ...
        public SoundPlayerSettings(string name, string caption, AudioClip clip, Vector3 position, float vol, bool loop = false, Transform parent = null, SoundPlayerSettings next = null, bool persist = false, bool music = false, float pitchMin = 1, float pitchMax = 1, bool randomizePitch = false, float spitch = 1, bool pitchSet = false)
        {
            soundName = name;
